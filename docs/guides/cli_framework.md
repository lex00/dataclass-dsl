...
Domain packages can use these utilities to build their CLIs with consistent patterns:
...
| `create_build_command()` | Create a 'build' command handler |
| `create_lint_command()` | Create a 'lint' command handler |
...
Registered resources (3):
  ...
  DataBucket: MyDomain::Storage::Bucket
  ProcessorFunction: MyDomain::Compute::Function
  ProcessorRole: MyDomain::IAM::Role
```
...
```python
from dataclass_dsl import create_build_command

build_cmd = create_build_command(
    registry=registry,
    template_class=MyDomainTemplate,
    default_format="yaml",
)
```

**Parameters:**
- `registry` (ResourceRegistry): Registry to build from
- `template_class`: Domain-specific template class
- `default_format` (str): Default output format ("json" or "yaml")

---

### create_lint_command()

Create a 'lint' command handler with optional auto-fix.

```python
from dataclass_dsl import create_lint_command

lint_cmd = create_lint_command(
    lint_file=my_lint_function,
    fix_file=my_fix_function,
    stub_config=my_stub_config,
)
```

**Parameters:**
- `lint_file` (Callable): Function that lints a file and returns issues
- `fix_file` (Callable): Function that fixes a file
- `stub_config` (StubConfig): Configuration for stub regeneration

---

## Domain-Specific Commands

The `build` command is typically domain-specific because it generates output in a particular format. Example pattern:
